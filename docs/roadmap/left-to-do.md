...
and then deleted). Copy source of truth is now `apps/web/content/*.mdx` (page bodies and
frontmatter) plus `apps/web/src/site.ts` (brand strings and chrome copy). Per-page titles
and `<head>` metadata (description, Open Graph, Twitter card) come from each page's MDX
frontmatter, derived by `apps/web/src/seo.ts` in the route files' `head` option.

## Launch blockers

- [ ] **Form endpoint** — `FORM_ENDPOINT` in `apps/web/src/config.ts` is a placeholder.
      ...
      and name the provider in the privacy copy (`content/pages/privacy.mdx`, "Where it
      goes." section). All three forms POST to the one endpoint with a hidden `form` field:
...
- Privacy page says "We don't run tracking or analytics scripts." — true of the repo
  ...
  `content/pages/privacy.mdx` accordingly.
- Form success/error microcopy in `FORM_MESSAGES` (`apps/web/src/site.ts`) was written during the
...
- [x] og:image (2026-06-07; build-time generation 2026-06-08) — `/og.png` (1200×630),
      ...
      from `apps/web/src/site.ts`, so it can't drift from the site; Inter ships via the
      `@fontsource/inter` dependency of `@zyplux/og`. The plugin emits the PNG into `dist/og.png` on every
